...
extension CoreDataExecutor {
    ...
    public static func configureShared(container: NSPersistentContainer) {
        _shared = CoreDataExecutor(
            container: ManagedObjectContextContainer(persistentContainer: container)
        )
    }
    ...
    public static var shared: CoreDataExecutor {
        guard let s = _shared else {
            preconditionFailure(
                "CoreDataExecutor is not configured. Call CoreDataExecutor.configureShared(container:) at app launch."
            )
        }
        return s
    }
